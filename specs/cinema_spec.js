...
describe('Cinema', function () {
  ...
  it('should be able to check whether there are no films from a particular year', function() {
    const actual = cinema.filmsByYear(1901);
    assert.deepStrictEqual(actual, false);
  });

  it('should be able to check whether all films are over a particular length', function() {
    const allFilms = cinema.allFilmLengthCheck(10);
    const someFilms = cinema.allFilmLengthCheck(100);
    const noFilms = cinema.allFilmLengthCheck(1000);
    assert.strictEqual(allFilms, true);
    assert.strictEqual(someFilms, false);
    assert.strictEqual(noFilms, false);
  });
